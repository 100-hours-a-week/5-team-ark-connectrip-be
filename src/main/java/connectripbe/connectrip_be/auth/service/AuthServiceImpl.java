...
public class AuthServiceImpl implements AuthService {
     ...
     * 사용자의 로그아웃 요청을 처리
     *
     * @param accessToken 로그아웃을 요청한 사용자의 accessToken 정보를 담은 객체
     */
    public void logout(String accessToken) {
        ...
        if (redisService.getData(REFRESH_TOKEN_PREFIX + accessToken) != null) {
            redisService.deleteData(REFRESH_TOKEN_PREFIX + accessToken);
        }

        // 액세스 토큰의 유효 시간을 확인하고 로그아웃한 토큰을 블랙리스트에 등록.
        Long expiredTime = tokenProvider.getExpireTime(accessToken);
        redisService.setDataExpire(REFRESH_TOKEN_PREFIX + accessToken,
                "Logout", expiredTime);
    }
